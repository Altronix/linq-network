...
MG_INTERNAL int mg_parse_address(
    const char* str,
    union socket_address* sa,
    int* proto,
    char* host,
    size_t host_len);
MG_INTERNAL void mg_call(
 ...
 */
MG_INTERNAL size_t mg_handle_chunked(
    struct mg_connection* nc,
...
MG_INTERNAL void mg_handle_cgi(
    struct mg_connection* nc,
    const char* prog,
...
MG_INTERNAL void mg_handle_ssi_request(
    struct mg_connection* nc,
    struct http_message* hm,
    const char* path,
    const struct mg_serve_http_opts* opts);
...
MG_INTERNAL void mg_handle_propfind(
    struct mg_connection* nc,
    const char* path,
    cs_stat_t* stp,
    struct http_message* hm,
...
MG_INTERNAL void mg_handle_mkcol(
    struct mg_connection* nc,
    const char* path,
    struct http_message* hm);
MG_INTERNAL void mg_handle_move(
    struct mg_connection* c,
    const struct mg_serve_http_opts* opts,
    const char* path,
    struct http_message* hm);
MG_INTERNAL void mg_handle_delete(
    struct mg_connection* nc,
    const struct mg_serve_http_opts* opts,
    const char* path);
MG_INTERNAL void mg_handle_put(
    struct mg_connection* nc,
    const char* path,
    struct http_message* hm);
#endif
#if MG_ENABLE_HTTP_WEBSOCKET
MG_INTERNAL void mg_ws_handler(
    struct mg_connection* nc,
    int ev,
    void* ev_data MG_UD_ARG(void* user_data));
MG_INTERNAL void mg_ws_handshake(
    struct mg_connection* nc,
...
cs_base64_emit_code(struct cs_base64_ctx* ctx, int v)
{
    if (v < NUM_UPPERCASES) {
        ...
        ctx->b64_putc(
            v - NUM_LETTERS - NUM_DIGITS == 0 ? '+' : '/', ctx->user_data);
    }
}
...
cs_base64_emit_chunk(struct cs_base64_ctx* ctx)
{
    ...
    if (ctx->chunk_size > 2) { cs_base64_emit_code(ctx, c & 63); }
}
...
#define BASE64_ENCODE_BODY                                                     \
    ...
    for (i = j = 0; i < src_len; i += 3) {                                     \
        ...
        if (i + 1 < src_len) { BASE64_OUT(b64[(b & 15) << 2 | (c >> 6)]); }    \
        if (i + 2 < src_len) { BASE64_OUT(b64[c & 63]); }                      \
    }                                                                          \
                                                                               \
    while (j % 4 != 0) { BASE64_OUT('='); }                                    \
    BASE64_FLUSH()
...
extern "C"
{
    ...
    /*
     * Log level; `LL_INFO` is the default. Use `cs_log_set_level()` to change
     * it.
     */
    enum cs_log_level
    {
    ...
    /*
     * Set max log level to print; messages with the level above the given one
     * will not be printed.
     */
     ...
     * prefix is matched against the log prefix exactly as printed, including
     * line number, but partial match is ok. Check stops on first matching
     * entry. If nothing matches, default level is used.
     *
     * Examples:
     *   main.c:=4 - everything from main C at verbose debug level.
     *   mongoose.c=1,mjs.c=1,=4 - everything at verbose debug except mg_* and
     * mjs_*
     *
     */
     ...
     */
    int
    cs_log_print_prefix(enum cs_log_level level, const char* fname, int line);
...
#define LOG(l, x)                                                              \
    do {                                                                       \
        if (cs_log_print_prefix(l, __FILE__, __LINE__)) { cs_log_printf x; }   \
    } while (0)
...
void
cs_log_set_file_level(const char* file_level)
{
...
int
cs_log_print_prefix(enum cs_log_level level, const char* file, int ln)
{
    ...
    ll =
        (ln < 10000 ? (ln < 1000 ? (ln < 100 ? (ln < 10 ? 1 : 2) : 3) : 4) : 5);
    ...
    *(--q) = ':';
    ...
    if (s_file_level != NULL) {
        ...
        struct mg_str fl = mg_mk_str(s_file_level),
                      ps = MG_MK_STR_N(prefix, pl);
        struct mg_str k, v;
...
#if CS_LOG_ENABLE_TS_DIFF
    {
        double now = cs_time();
        fprintf(
            cs_log_file, "%7u ", (unsigned int)((now - cs_log_ts) * 1000000));
        cs_log_ts = now;
    }
#endif
    return 1;
...
static void
cs_md5_transform(uint32_t buf[4], uint32_t const in[16])
{
...
void
cs_md5_update(cs_md5_ctx* ctx, const unsigned char* buf, size_t len)
{
    uint32_t t;
...
void
cs_md5_final(unsigned char digest[16], cs_md5_ctx* ctx)
{
    unsigned count;
    unsigned char* p;
    uint32_t* a;
...
void
cs_sha1_update(cs_sha1_ctx* context, const unsigned char* data, uint32_t len)
{
    uint32_t i, j;
...
size_t
mbuf_append_and_free(struct mbuf* a, void* data, size_t len)
{
    size_t ret;
...
void
mbuf_move(struct mbuf* from, struct mbuf* to)
{
...
mg_strdup_common(const struct mg_str s, int nul_terminate)
{
...
struct mg_str
mg_strdup_nul(const struct mg_str s)
{
...
const char*
mg_strchr(const struct mg_str s, int c)
{
    size_t i;
    for (i = 0; i < s.len; i++) {
...
mg_strncmp(const struct mg_str str1, const struct mg_str str2, size_t n)
{
    ...
    if (s1.len > n) { s1.len = n; }
    if (s2.len > n) { s2.len = n; }
    return mg_strcmp(s1, s2);
}

void mg_strfree(struct mg_str* s) WEAK;
void
mg_strfree(struct mg_str* s)
{
    char* sp = (char*)s->p;
    s->p = NULL;
    s->len = 0;
...
const char* mg_strstr(const struct mg_str haystack, const struct mg_str needle)
    WEAK;
const char*
mg_strstr(const struct mg_str haystack, const struct mg_str needle)
{
    size_t i;
...
struct mg_str
mg_strstrip(struct mg_str s)
{
    while (s.len > 0 && isspace((int)*s.p)) {
        s.p++;
        s.len--;
    }
    while (s.len > 0 && isspace((int)*(s.p + s.len - 1))) { s.len--; }
    return s;
}

int mg_str_starts_with(struct mg_str s, struct mg_str prefix) WEAK;
int
mg_str_starts_with(struct mg_str s, struct mg_str prefix)
{
    ...
    int field_width)
{
    ...
    if (flags && C_SNPRINTF_FLAG_ZERO) {
        while (k < field_width && k < (int)sizeof(tmp) - 1) { tmp[k++] = '0'; }
    }
    ...
    /* Now output */
    while (--k >= 0) { C_SNPRINTF_APPEND_CHAR(tmp[k]); }
...
c_vsnprintf(char* buf, size_t buf_size, const char* fmt, va_list ap)
{
    ...
    while ((ch = *fmt++) != '\0') {
        if (ch != '%') {
            ...
            if (*fmt == '0') { flags |= C_SNPRINTF_FLAG_ZERO; }
            ...
            if (ch == 's') {
                ...
                for (j = 0; j < pad; j++) { C_SNPRINTF_APPEND_CHAR(' '); }

                /* `s` may be NULL in case of %.*s */
                if (s != NULL) {
                    /* Ignore negative and 0 precisions */
                    for (j = 0;
                         (precision <= 0 || j < precision) && s[j] != '\0';
                         j++) {
    ...
    if (buf_size > 0) { buf[i < (int)buf_size ? i : (int)buf_size - 1] = '\0'; }
...
c_strnstr(const char* s, const char* find, size_t slen)
{
    ...
    for (i = 0; i < slen; i++) {
        if (i + find_length > slen) { return NULL; }

        if (strncmp(&s[i], find, find_length) == 0) { return &s[i]; }
    }
...
mg_avprintf(char** buf, size_t size, const char* fmt, va_list ap)
{
    ...
    if (len < 0) {
        ...
        *buf = NULL; /* LCOV_EXCL_START */
        ...
        /*
         * Microsoft version of vsnprintf() is not always null-terminated, so
         * put the terminator manually
         */
        (*buf)[len] = 0;
        /* LCOV_EXCL_STOP */
...
const char*
mg_next_comma_list_entry(const char*, struct mg_str*, struct mg_str*) WEAK;
const char*
...
{
    struct mg_str ret =
        mg_next_comma_list_entry_n(mg_mk_str(list), val, eq_val);
    return ret.p;
}
...
mg_match_prefix_n(const struct mg_str pattern, const struct mg_str str)
{
    ...
    for (; i < pattern.len && j < str.len; i++, j++) {
        if (pattern.p[i] == '?') {
            ...
            if (i == pattern.len ||
                (pattern.p[i] == '$' && i == pattern.len - 1))
                return j + len;
            do {
                const struct mg_str pstr = { pattern.p + i, pattern.len - i };
                const struct mg_str sstr = { str.p + j + len,
                                             str.len - j - len };
                res = mg_match_prefix_n(pstr, sstr);
            } while (res == 0 && len != 0 && len-- > 0);
            return res == 0 ? 0 : j + res + len;
        } else if (
            str_util_lowercase(&pattern.p[i]) !=
...
mg_add_conn(struct mg_mgr* mgr, struct mg_connection* c)
{
    ...
    if (c->sock != MG_INVALID_SOCKET) { c->iface->vtable->add_conn(c); }
}
    ...
    void* ev_data)
{
    ...
    if (ev != MG_EV_POLL) {
        DBG(
            ("%p %s ev=%d ev_data=%p flags=0x%lx rmbl=%d smbl=%d",
             nc,
             ev_handler == nc->handler ? "user" : "proto",
             ev,
             ev_data,
             nc->flags,
...
MG_INTERNAL void
mg_timer(struct mg_connection* c, double now)
{
...
int
mg_if_poll(struct mg_connection* nc, double now)
{
...
#if MG_ENABLE_SSL
    if ((nc->flags & (MG_F_SSL | MG_F_LISTENING | MG_F_CONNECTING)) ==
        MG_F_SSL) {
...
{
    if (conn->sock != MG_INVALID_SOCKET) { /* Don't print timer-only conns */
...
mg_close_conn(struct mg_connection* conn)
{
    ...
    if (conn->sock != MG_INVALID_SOCKET && mg_do_recv(conn) == -2) {
...
void
mg_mgr_init_opt(struct mg_mgr* m, void* user_data, struct mg_mgr_init_opts opts)
{
    memset(m, 0, sizeof(*m));
#if MG_ENABLE_BROADCAST
    m->ctl[0] = m->ctl[1] = MG_INVALID_SOCKET;
#endif
    ...
    if (opts.nameserver != NULL) { m->nameserver = strdup(opts.nameserver); }
    DBG(("=================================="));
    DBG(("init mgr=%p", m));
#if MG_ENABLE_SSL
    {
...
#if MG_ENABLE_BROADCAST
    if (m->ctl[0] != MG_INVALID_SOCKET) mg_closesocket(m->ctl[0]);
    if (m->ctl[1] != MG_INVALID_SOCKET) mg_closesocket(m->ctl[1]);
    m->ctl[0] = m->ctl[1] = MG_INVALID_SOCKET;
#endif
    ...
    size_t host_len)
{
    ...
    *proto = SOCK_STREAM;
    ...
    if (sscanf(str, "%u.%u.%u.%u:%u%n", &a, &b, &c, &d, &port, &len) == 5) {
        ...
        sscanf(str, "[%99[^]]]:%u%n", buf, &port, &len) == 2 &&
        ...
        if (mg_resolve_from_hosts_file(host, sa) != 0) {
             ...
             */
            if (mg_ncasecmp(host, "localhost", 9) != 0) { return 0; }

#if MG_ENABLE_SYNC_RESOLVER
            if (!mg_resolve2(host, &sa->sin.sin_addr)) { return -1; }
#else
            return -1;
#endif
        }
...
#if MG_ENABLE_SSL
MG_INTERNAL void
mg_ssl_handshake(struct mg_connection* nc)
{
    int err = 0;
...
static int
mg_do_recv(struct mg_connection* nc)
{
    int res = 0;
    char* buf = NULL;
...
void
mg_if_can_recv_cb(struct mg_connection* nc)
{
    mg_do_recv(nc);
}

static int
mg_recv_tcp(struct mg_connection* nc, char* buf, size_t len)
{
    int n = 0;
#if MG_ENABLE_SSL
    ...
    if (n > 0) {
        ...
        if (nc->mgr && nc->mgr->hexdump_file != NULL) {
            mg_hexdump_connection(
                nc, nc->mgr->hexdump_file, buf, n, MG_EV_RECV);
        }
#endif
...
static int
mg_recv_udp(struct mg_connection* nc, char* buf, size_t len)
{
    int n = 0;
    ...
    if (nc->flags & MG_F_LISTENING) {
        ...
        if (nc == NULL) {
            ...
            if (nc != NULL) {
                ...
                nc->flags = MG_F_UDP;
                /*
                 * Long-lived UDP "connections" i.e. interactions that involve
                 * more than one request and response are rare, most are
                 * transactional: response is sent and the "connection" is
                 * closed. Or - should be. But users (including ourselves) tend
                 * to forget about that part, because UDP is connectionless and
                 * one does not think about processing a UDP request as handling
                 * a connection that needs to be closed. Thus, we begin with
                 * SEND_AND_CLOSE flag set, which should be a reasonable default
                 * for most use cases, but it is possible to turn it off the
                 * connection should be kept alive after processing.
                 */
...
#endif
        if (n != 0) { mg_call(nc, NULL, nc->user_data, MG_EV_RECV, &n); }
    }
...
void
mg_if_can_send_cb(struct mg_connection* nc)
{
    int n = 0;
    const char* buf = nc->send_mbuf.buf;
    size_t len = nc->send_mbuf.len;

    if (nc->flags & (MG_F_CLOSE_IMMEDIATELY | MG_F_CONNECTING)) { return; }
    if (!(nc->flags & MG_F_UDP)) {
...
mg_if_connect_cb(struct mg_connection* nc, int err)
{
    LOG(LL_DEBUG,
        ("%p %s://%s:%hu -> %d",
         ...
         err));
    nc->flags &= ~MG_F_CONNECTING;
    if (err != 0) { nc->flags |= MG_F_CLOSE_IMMEDIATELY; }
#if MG_ENABLE_SSL
        ...
        for (i = 0; i < msg->num_answers; i++) {
            if (msg->answers[i].rtype == MG_DNS_A_RECORD) {
                ...
                mg_dns_parse_record_data(
                    msg, &msg->answers[i], &nc->sa.sin.sin_addr, 4);
                mg_do_connect(
                    nc,
...
#if MG_ENABLE_BROADCAST
void
mg_broadcast(struct mg_mgr* mgr, mg_event_handler_t cb, void* data, size_t len)
{
     ...
     */
    if (mgr->ctl[0] != MG_INVALID_SOCKET && data != NULL &&
        ...
        dummy = MG_SEND_FUNC(
            mgr->ctl[0],
            (char*)&ctl_msg,
            offsetof(struct ctl_msg, message) + len,
            0);
...
{
    if (sock != MG_INVALID_SOCKET) { nc->iface->vtable->sock_set(nc, sock); }
}
...
struct mg_iface*
mg_if_create_iface(const struct mg_iface_vtable* vtable, struct mg_mgr* mgr)
{
    ...
    struct mg_iface* from)
{
    ...
    for (; i < mgr->num_ifaces; i++) {
        if (mgr->ifaces[i]->vtable == vtable) { return mgr->ifaces[i]; }
    }
    return NULL;
}

double
mg_mgr_min_timer(const struct mg_mgr* mgr)
{
    double min_timer = 0;
    struct mg_connection* nc;
...
static void
mg_null_if_connect_tcp(struct mg_connection* c, const union socket_address* sa)
{
...
static void
mg_null_if_connect_udp(struct mg_connection* c)
{
    c->flags |= MG_F_CLOSE_IMMEDIATELY;
}

static int
mg_null_if_listen_tcp(struct mg_connection* c, union socket_address* sa)
{
    (void)c;
    (void)sa;
    return -1;
...
static int
mg_null_if_listen_udp(struct mg_connection* c, union socket_address* sa)
{
    (void)c;
    (void)sa;
    return -1;
...
static int
mg_null_if_tcp_send(struct mg_connection* c, const void* buf, size_t len)
{
    (void)c;
    (void)buf;
    (void)len;
...
static int
mg_null_if_udp_send(struct mg_connection* c, const void* buf, size_t len)
{
    (void)c;
    (void)buf;
    (void)len;
...
int
mg_null_if_tcp_recv(struct mg_connection* c, void* buf, size_t len)
{
    (void)c;
    (void)buf;
    (void)len;
...
static int
mg_null_if_create_conn(struct mg_connection* c)
{
    (void)c;
    return 1;
}
...
static void
mg_null_if_destroy_conn(struct mg_connection* c)
{
    (void)c;
}

static void
mg_null_if_sock_set(struct mg_connection* c, sock_t sock)
{
    (void)c;
    (void)sock;
}
...
static void
mg_null_if_init(struct mg_iface* iface)
{
    (void)iface;
}

static void
mg_null_if_free(struct mg_iface* iface)
{
    (void)iface;
}

static void
mg_null_if_add_conn(struct mg_connection* c)
{
    c->sock = MG_INVALID_SOCKET;
    c->flags |= MG_F_CLOSE_IMMEDIATELY;
}

static void
mg_null_if_remove_conn(struct mg_connection* c)
{
    (void)c;
}

static time_t
mg_null_if_poll(struct mg_iface* iface, int timeout_ms)
{
...
#define MG_NULL_IFACE_VTABLE                                                   \
    {                                                                          \
        mg_null_if_init, mg_null_if_free, mg_null_if_add_conn,                 \
            ...
            mg_null_if_listen_udp, mg_null_if_connect_tcp,                     \
            mg_null_if_connect_udp, mg_null_if_tcp_send, mg_null_if_udp_send,  \
            mg_null_if_tcp_recv, mg_null_if_udp_recv, mg_null_if_create_conn,  \
            mg_null_if_destroy_conn, mg_null_if_sock_set,                      \
            mg_null_if_get_conn_addr,                                          \
    }
...
mg_socket_if_listen_udp(struct mg_connection* nc, union socket_address* sa)
{
    ...
    if (sock == MG_INVALID_SOCKET) return (mg_get_errno() ? mg_get_errno() : 1);
    mg_sock_set(nc, sock);
    return 0;
}
...
static int
mg_socket_if_tcp_recv(struct mg_connection* nc, void* buf, size_t len)
{
...
{
    if (nc->sock == MG_INVALID_SOCKET) return;
    if (!(nc->flags & MG_F_UDP)) {
        mg_closesocket(nc->sock);
    } else {
        /* Only close outgoing UDP sockets or listeners. */
        if (nc->listener == NULL) mg_closesocket(nc->sock);
    }
    nc->sock = MG_INVALID_SOCKET;
}
...
mg_open_listening_socket(union socket_address* sa, int type, int proto)
{
    ...
    sock_t sock = MG_INVALID_SOCKET;
#if !MG_LWIP
    int on = 1;
#endif

    if ((sock = socket(sa->sa.sa_family, type, proto)) != MG_INVALID_SOCKET &&
#if !MG_LWIP /* LWIP doesn't support either */
#if defined(_WIN32) && defined(SO_EXCLUSIVEADDRUSE) && !defined(WINCE)
        /* "Using SO_REUSEADDR and SO_EXCLUSIVEADDRUSE" http://goo.gl/RmrFTm */
        !setsockopt(
            sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (void*)&on, sizeof(on)) &&
#endif
...
#endif
    } else if (sock != MG_INVALID_SOCKET) {
        mg_closesocket(sock);
        sock = MG_INVALID_SOCKET;
    }
    ...
    if (nc->flags & MG_F_CONNECTING) {
        if (fd_flags != 0) {
            ...
            if (!(nc->flags & MG_F_UDP)) {
                ...
                int ret = getsockopt(
                    nc->sock, SOL_SOCKET, SO_ERROR, (char*)&err, &len);
                if (ret != 0) {
                    err = 1;
...
mg_mgr_handle_ctl_sock(struct mg_mgr* mgr)
{
    ...
    if (len >= (int)sizeof(ctl_msg.callback) && ctl_msg.callback != NULL) {
        ...
        for (nc = mg_next(mgr, NULL); nc != NULL; nc = mg_next(mgr, nc)) {
            ctl_msg.callback(
                nc, MG_EV_POLL, ctl_msg.message MG_UD_ARG(nc->user_data));
        }
    }
}
#endif
...
mg_add_to_set(sock_t sock, fd_set* set, sock_t* max_fd)
{
    if (sock != MG_INVALID_SOCKET
        ...
        FD_SET(sock, set);
        if (*max_fd == MG_INVALID_SOCKET || sock > *max_fd) { *max_fd = sock; }
    }
}
...
mg_socket_if_poll(struct mg_iface* iface, int timeout_ms)
{
    ...
    sock_t max_fd = MG_INVALID_SOCKET;
    ...
    /*
     * Note: it is ok to have connections with sock == MG_INVALID_SOCKET in the
            ...
            if (nc->sock >= (sock_t)FD_SETSIZE && try_dup) {
                ...
                if (new_sock >= 0) {
                    if (new_sock < (sock_t)FD_SETSIZE) {
                        ...
                        mg_closesocket(new_sock);
                        DBG(("new sock is still larger than FD_SETSIZE, "
                             "disregard"));
                        try_dup = 0;
                    }
                } else {
...
#if MG_ENABLE_BROADCAST
    if (num_ev > 0 && mgr->ctl[1] != MG_INVALID_SOCKET &&
...
#if MG_LWIP
            /* With LWIP socket emulation layer, we don't get write events for
             * UDP */
...
#if MG_ENABLE_BROADCAST
MG_INTERNAL void
mg_socketpair_close(sock_t* sock)
{
    while (1) {
        if (mg_closesocket(*sock) == -1 && errno == EINTR) continue;
        break;
    }
    *sock = MG_INVALID_SOCKET;
}
...
mg_socketpair_accept(sock_t sock, union socket_address* sa, socklen_t sa_len)
{
    sock_t rc;
    while (1) {
        if ((rc = accept(sock, &sa->sa, &sa_len)) == MG_INVALID_SOCKET &&
            errno == EINTR)
            continue;
        break;
    }
...
mg_socketpair(sock_t sp[2], int sock_type)
{
    ...
    if ((sock = socket(AF_INET, sock_type, 0)) == MG_INVALID_SOCKET) {
    } else if (bind(sock, &sa.sa, len) != 0) {
    } else if (sock_type == SOCK_STREAM && listen(sock, 1) != 0) {
    } else if (getsockname(sock, &sa.sa, &len) != 0) {
    } else if ((sp[0] = socket(AF_INET, sock_type, 0)) == MG_INVALID_SOCKET) {
    } else if (sock_type == SOCK_STREAM && connect(sp[0], &sa.sa, len) != 0) {
    } else if (
        sock_type == SOCK_DGRAM && (bind(sp[0], &sa2.sa, len) != 0 ||
    ...
    if (!ret) {
        if (sp[0] != MG_INVALID_SOCKET) mg_socketpair_close(&sp[0]);
        if (sp[1] != MG_INVALID_SOCKET) mg_socketpair_close(&sp[1]);
        if (sock != MG_INVALID_SOCKET) mg_socketpair_close(&sock);
    }
...
static void
socks_if_disband(struct socksdata* d)
{
...
static void
socks_if_relay(struct mg_connection* s)
{
    ...
    if (d->c->send_mbuf.len > 0 && s->send_mbuf.len == 0)
        mg_if_can_send_cb(d->c);
}

static void
socks_if_handler(struct mg_connection* c, int ev, void* ev_data)
{
    ...
    if (ev == MG_EV_CONNECT) {
        ...
        if (res == 0) {
            ...
            unsigned char buf[] = { MG_SOCKS_VERSION,
                                    1,
        ...
        if (!(c->flags & MG_SOCKS_HANDSHAKE_DONE)) {
            ...
            unsigned char buf[10] = {
                MG_SOCKS_VERSION, MG_SOCKS_CMD_CONNECT, 0, MG_SOCKS_ADDR_IPV4
            };
            if (c->recv_mbuf.len < 2) return;
            if ((unsigned char)c->recv_mbuf.buf[1] ==
        ...
        if ((c->flags & MG_SOCKS_HANDSHAKE_DONE) &&
            ...
            if (c->recv_mbuf.buf[1] != MG_SOCKS_SUCCESS) {
                LOG(LL_ERROR,
                    ("Socks connection error: %d", c->recv_mbuf.buf[1]));
                socks_if_disband(d);
                return;
            }
...
static void
mg_socks_if_connect_tcp(struct mg_connection* c, const union socket_address* sa)
{
...
static void
mg_socks_if_connect_udp(struct mg_connection* c)
{
    (void)c;
}

static int
mg_socks_if_listen_tcp(struct mg_connection* c, union socket_address* sa)
{
    (void)c;
    (void)sa;
    return 0;
...
static int
mg_socks_if_listen_udp(struct mg_connection* c, union socket_address* sa)
{
    (void)c;
    (void)sa;
    return -1;
...
static int
mg_socks_if_tcp_send(struct mg_connection* c, const void* buf, size_t len)
{
    int res;
    struct socksdata* d = (struct socksdata*)c->iface->data;
...
static int
mg_socks_if_udp_send(struct mg_connection* c, const void* buf, size_t len)
{
    (void)c;
    (void)buf;
    (void)len;
...
int
mg_socks_if_tcp_recv(struct mg_connection* c, void* buf, size_t len)
{
    struct socksdata* d = (struct socksdata*)c->iface->data;
    if (d->s == NULL) return -1;
...
static int
mg_socks_if_create_conn(struct mg_connection* c)
{
    (void)c;
    return 1;
}
...
static void
mg_socks_if_destroy_conn(struct mg_connection* c)
{
...
static void
mg_socks_if_sock_set(struct mg_connection* c, sock_t sock)
{
    (void)c;
    (void)sock;
}
...
static void
mg_socks_if_init(struct mg_iface* iface)
{
    (void)iface;
}

static void
mg_socks_if_free(struct mg_iface* iface)
{
...
static void
mg_socks_if_add_conn(struct mg_connection* c)
{
    c->sock = MG_INVALID_SOCKET;
}

static void
mg_socks_if_remove_conn(struct mg_connection* c)
{
    (void)c;
}

static time_t
mg_socks_if_poll(struct mg_iface* iface, int timeout_ms)
{
    LOG(LL_DEBUG, ("%p", iface));
    (void)iface;
    (void)timeout_ms;
...
struct mg_iface*
mg_socks_mk_iface(struct mg_mgr* mgr, const char* proxy_addr)
{
...
mg_ssl_if_conn_accept(struct mg_connection* nc, struct mg_connection* lc)
{
    ...
    if ((ctx->ssl = SSL_new(ctx->ssl_ctx)) == NULL) { return MG_SSL_ERROR; }
    return MG_SSL_OK;
}

static enum mg_ssl_if_result mg_use_cert(
    SSL_CTX* ctx,
    const char* cert,
    const char* key,
    const char** err_msg);
...
static enum mg_ssl_if_result mg_ssl_if_ossl_set_psk(
    struct mg_ssl_if_ctx* ctx,
    const char* identity,
    const char* key_str);
...
void
mg_ssl_if_conn_close_notify(struct mg_connection* nc)
{
    struct mg_ssl_if_ctx* ctx = (struct mg_ssl_if_ctx*)nc->ssl_if_data;
    if (ctx == NULL) return;
...
{
    if (cert == NULL || strcmp(cert, "*") == 0) { return MG_SSL_OK; }
    SSL_CTX_set_verify(
...
{
    return (
        SSL_CTX_set_cipher_list(ctx, cl ? cl : mg_s_cipher_list) == 1
            ? MG_SSL_OK
            : MG_SSL_ERROR);
}
    ...
    const char* str)
{
    ...
    switch (level) {
        case 1: cs_level = LL_ERROR; break;
        case 2: cs_level = LL_INFO; break;
        case 3: cs_level = LL_DEBUG; break;
        default: cs_level = LL_VERBOSE_DEBUG;
    }
...
mg_ssl_if_conn_accept(struct mg_connection* nc, struct mg_connection* lc)
{
    ...
    if (mbedtls_ssl_setup(ctx->ssl, lc_ctx->conf) != 0) { return MG_SSL_ERROR; }
    return MG_SSL_OK;
}
...
static enum mg_ssl_if_result mg_use_cert(
    ...
    const char* key,
    const char** err_msg);
static enum mg_ssl_if_result mg_use_ca_cert(
    struct mg_ssl_if_ctx* ctx,
    const char* cert);
    ...
    const char** err_msg)
{
    ...
    mbedtls_ssl_conf_min_version(
        ctx->conf, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
    ...
    if (mbedtls_ssl_conf_max_frag_len(
            ctx->conf,
...
static int
mg_ssl_if_mbed_send(void* ctx, const unsigned char* buf, size_t len)
{
...
static int
mg_ssl_if_mbed_recv(void* ctx, unsigned char* buf, size_t len)
{
    struct mg_connection* nc = (struct mg_connection*)ctx;
...
mg_ssl_if_handshake(struct mg_connection* nc)
{
    ...
    if (ctx->ssl->p_bio == NULL) {
        mbedtls_ssl_set_bio(
            ctx->ssl, nc, mg_ssl_if_mbed_send, mg_ssl_if_mbed_recv, NULL);
    }
    ...
    if (nc->listener == NULL) {
        if (ctx->conf->key_cert != NULL) {
            /* Note that this assumes one key_cert entry, which matches our
             * init. */
...
int
mg_ssl_if_read(struct mg_connection* nc, void* buf, size_t len)
{
    struct mg_ssl_if_ctx* ctx = (struct mg_ssl_if_ctx*)nc->ssl_if_data;
...
int
mg_ssl_if_write(struct mg_connection* nc, const void* buf, size_t len)
{
    struct mg_ssl_if_ctx* ctx = (struct mg_ssl_if_ctx*)nc->ssl_if_data;
...
mg_set_cipher_list(struct mg_ssl_if_ctx* ctx, const char* ciphers)
{
    if (ciphers != NULL) {
        ...
        while (s != NULL) {
            ...
            DBG(("%s -> %04x", tmp, id));
            if (id != 0) { mbuf_append(&ctx->cipher_suites, &id, sizeof(id)); }
            s = (e != NULL ? e + 1 : NULL);
        }
        ...
        mbedtls_ssl_conf_ciphersuites(
            ctx->conf, (const int*)ctx->cipher_suites.buf);
    } else {
 ...
 */
MG_INTERNAL void
altbuf_init(struct altbuf* ab, char* buf, size_t buf_size)
{
    mbuf_init(&ab->m, 0);
    ab->user_buf = buf;
 ...
 */
MG_INTERNAL void
altbuf_append(struct altbuf* ab, char c)
{
    if (ab->len < ab->user_buf_size) {
        ...
        /*
         * First of all, see if that's the first byte which overflows the
         * original buffer: if so, copy the existing data from there to a newly
         * allocated mbuf.
         */
 ...
 */
MG_INTERNAL int
altbuf_reallocated(struct altbuf* ab)
{
 ...
 */
MG_INTERNAL char*
altbuf_get_buf(struct altbuf* ab, int trim)
{
    if (altbuf_reallocated(ab)) {
        if (trim) { mbuf_trim(&ab->m); }
        return ab->m.buf;
    } else {
        return ab->user_buf;
    }
...
MG_INTERNAL struct mg_http_proto_data*
mg_http_create_proto_data(struct mg_connection* c)
{
...
static void
mg_http_proto_data_destructor(void* proto_data)
{
    ...
    const struct mg_serve_http_opts* opts)
{
    ...
    while ((overrides = mg_next_comma_list_entry(overrides, &k, &v)) != NULL) {
        ...
        if (path_len > k.len && mg_vcasecmp(&k, ext) == 0) { return v; }
    }
 ...
 */
static size_t
mg_http_parse_chunk(char* buf, size_t len, char** chunk_data, size_t* chunk_len)
{
...
struct mg_http_endpoint*
mg_http_get_endpoint_handler(struct mg_connection* nc, struct mg_str* uri_path)
{
    struct mg_http_proto_data* pd;
...
static void mg_http_multipart_begin(
    struct mg_connection* nc,
    struct http_message* hm,
    int req_len);
...
static void mg_http_call_endpoint_handler(
    struct mg_connection* nc,
    int ev,
    struct http_message* hm);

static void
deliver_chunk(struct mg_connection* c, struct http_message* hm, int req_len)
{
...
#if MG_ENABLE_FILESYSTEM
    if (pd != NULL && pd->file.fp != NULL) { mg_http_transfer_file_data(nc); }
#endif
    ...
    again:
        ...
        if ((req_len < 0 ||
            ...
            if (!(nc->flags & (MG_F_CLOSE_IMMEDIATELY | MG_F_SEND_AND_CLOSE))) {
                if (nc->send_mbuf.len == 0) { mg_ws_handshake(nc, vec, hm); }
                mg_call(
                    nc,
                    nc->handler,
                    nc->user_data,
            ...
            if (nc->recv_mbuf_limit > 0 &&
                ...
                LOG(LL_ERROR,
                    ("%p recv buffer (%lu bytes) exceeds the limit "
                     ...
                     nc,
                     (unsigned long)nc->recv_mbuf.len,
...
#if MG_ENABLE_FILESYSTEM
            /* We don't have a generic mechanism of communicating that we are
             * done responding to a request (should probably add one). But if we
             * are serving a file, we are definitely not done. */
    ...
    int req_len)
{
    ...
    if (ct->len < 9 || strncmp(ct->p, "multipart", 9) != 0) { goto exit_mp; }
    ...
    if (pd->mp_stream.boundary != NULL) {
        ...
        ep = mg_http_get_endpoint_handler(nc->listener, &hm->uri);
        if (ep != NULL) { pd->endpoint_handler = ep->handler; }
...
mg_http_multipart_wait_for_boundary(struct mg_connection* c)
{
    ...
    if (boundary != NULL) {
        ...
        if (io->len - (boundary_end - io->buf) < 4) { return 0; }
...
static void mg_http_parse_header_internal(
    struct mg_str* hdr,
    const char* var_name,
    struct altbuf* ab);
    ...
    while (data_size > 0 &&
        ...
        if (line_len == 2 && mg_ncasecmp(block_begin, "\r\n", 2) == 0) {
            ...
            pd->mp_stream.file_name =
                altbuf_get_buf(&ab_file_name, 1 /* trim */);
            MG_FREE((void*)pd->mp_stream.var_name);
...
mg_http_multipart_continue_wait_for_chunk(struct mg_connection* c)
{
    ...
    if (boundary == NULL) {
        ...
        size_t consumed = mg_http_multipart_call_handler(
            c, MG_EV_HTTP_PART_DATA, io->buf, data_len);
        mbuf_remove(io, consumed);
    ...
    while (1) {
        switch (pd->mp_stream.state) {
            ...
            case MPS_WAITING_FOR_BOUNDARY: {
                if (mg_http_multipart_wait_for_boundary(c) == 0) { return; }
                break;
            }
            case MPS_GOT_BOUNDARY: {
                if (mg_http_multipart_process_boundary(c) == 0) { return; }
                break;
            }
            ...
            case MPS_FINALIZE: {
                if (mg_http_multipart_finalize(c) == 0) { return; }
                break;
            }
            case MPS_FINISHED: {
                return;
...
static int
mg_http_parse_range_header(const struct mg_str* header, int64_t* a, int64_t* b)
{
    /*
    ...
    const struct mg_str extra_headers)
{
    ...
    _XOPEN_SOURCE >= 600
        ...
        mg_printf(
            ...
            (int)mime_type.len,
            mime_type.p,
            (pd->file.keepalive ? "keep-alive" : "close"),
            (size_t)cl,
            range,
            etag);
...
mg_printf_http_chunk(struct mg_connection* nc, const char* fmt, ...)
{
    ...
    if (len >= 0) { mg_send_http_chunk(nc, buf, len); }

    /* LCOV_EXCL_START */
    if (buf != mem && buf != NULL) { MG_FREE(buf); }
    /* LCOV_EXCL_STOP */
}
    ...
    struct altbuf* ab)
{
    ...
    if (s != NULL && &s[n + 1] < end) {
        ...
        if (ch != ' ' && *p != ch) { altbuf_reset(ab); }
    }

    /* If there is some data, append a NUL. */
    if (ab->len > 0) { altbuf_append(ab, '\0'); }
}
    ...
    size_t pass_len)
{
    ...
    snprintf(
        ...
        user_len - 1,
        pass_len - 1);
    if (sscanf(buf, fmt, user, pass) == 0) { res = -1; }
    ...
    const char* nonce)
{
    ...
    cs_md5(
        ...
        one,
        auth_domain,
        (size_t)strlen(auth_domain),
        colon,
        one,
        passwd,
        (size_t)strlen(passwd),
        NULL);
    mg_mkmd5resp(
        method,
        strlen(method),
    ...
    FILE* fp)
{
    ...
    char *username = username_buf, *cnonce = cnonce_buf,
         *response = response_buf, *uri = uri_buf, *qop = qop_buf, *nc = nc_buf,
         *nonce = nonce_buf;
    ...
    if (hm == NULL || fp == NULL ||
        ...
        mg_http_parse_header2(
            hdr, "username", &username, sizeof(username_buf)) == 0 ||
        mg_http_parse_header2(hdr, "cnonce", &cnonce, sizeof(cnonce_buf)) ==
            0 ||
        mg_http_parse_header2(
            hdr, "response", &response, sizeof(response_buf)) == 0 ||
    ...
    *index_file = NULL;
    ...
    while ((list = mg_next_comma_list_entry(list, &vec, NULL)) != NULL) {
        ...
        *index_file = (char*)MG_REALLOC(*index_file, len);
        ...
        snprintf(
            *index_file, len, "%s%c%.*s", path, DIRSEP, (int)vec.len, vec.p);
    ...
    const struct mg_serve_http_opts* opts)
{
    ...
    mg_conn_addr_to_str(
        c, local_port + 1, sizeof(local_port) - 1, MG_SOCK_STRINGIFY_PORT);
    ...
    void* ev_data MG_UD_ARG(void* user_data))
{
    ...
    switch (ev) {
        case MG_EV_CONNECT:
            if (*(int*)ev_data != 0) {
                mg_http_send_error(
                    pd->reverse_proxy_data.linked_conn, 502, NULL);
            }
            break;
    ...
    struct mg_str upstream)
{
    ...
    const char* error;
    struct mg_connect_opts opts;
    struct mg_str path = MG_NULL_STR, user_info = MG_NULL_STR,
                  host = MG_NULL_STR;
    memset(&opts, 0, sizeof(opts));
    ...
    mg_asprintf(
        ...
        (int)upstream.len,
        upstream.p,
        (int)(hm->uri.len - mount.len),
        hm->uri.p + mount.len);

    be = mg_connect_http_base(
        nc->mgr,
        MG_CB(mg_reverse_proxy_handler, NULL),
        opts,
        "http",
        NULL,
        "https",
    ...
    for (i = 0; i < MG_MAX_HTTP_HEADERS && hm->header_names[i].len > 0; i++) {
        ...
        /*
         * Don't pass chunked transfer encoding to the client because hm->body
         * is already dechunked when we arrive here.
         */
        ...
        *p = '\0';
        ...
        while (u <= cp_end) {
            ...
            if (component.len > 0) {
                ...
                len = mg_url_decode(
                    p + 1, component.len, p + 1, lp_end - p - 1, 0);
                if (len <= 0) {
                    ok = 0;
                    break;
                }
...
{
    static const unsigned short days_before_month[] = { 0,   31,  59,  90,
                                                        120, 151, 181, 212,
                                                        243, 273, 304, 334 };
    char month_str[32];
...
static void
mg_http_send_options(struct mg_connection* nc, struct mg_serve_http_opts* opts)
{
    mg_send_response_line(nc, 200, opts->extra_headers);
    mg_printf(
        nc,
    ...
    struct mg_serve_http_opts* opts)
{
    ...
    if (is_dav && opts->dav_document_root == NULL) {
        ...
        (!exists || mg_is_file_hidden(path, opts, 0 /* specials are ok */)) &&
        ...
        is_dav && (opts->dav_auth_file == NULL ||
    ...
    struct mg_serve_http_opts opts)
{
...
#if MG_ENABLE_HTTP_URL_REWRITES
    if (mg_http_handle_forwarding(nc, hm, &opts)) { return; }

    if (mg_http_send_port_based_redirect(nc, hm, &opts)) { return; }
#endif

    if (opts.document_root == NULL) { opts.document_root = "."; }
    ...
    if (opts.ssi_pattern == NULL) { opts.ssi_pattern = "**.shtml$|**.shtm$"; }
...
mg_file_upload_handler(
    ...
    int ev,
    void* ev_data,
    mg_fu_fname_fn local_name_fn MG_UD_ARG(void* user_data))
{
    switch (ev) {
        case MG_EV_HTTP_PART_BEGIN: {
            ...
            if (fus->fp == NULL) {
                ...
                mg_printf(
                    nc, "Failed to open %s: %d\n", fus->lfn, mg_get_errno());
                /* Do not close the connection just yet, discard remainder of
                 * the data. This is because at the time of writing some
                 * browsers (Chrome) fail to
        ...
        case MG_EV_HTTP_PART_DATA: {
            ...
            if (mg_fwrite(mp->data.p, 1, mp->data.len, fus->fp) !=
                mp->data.len) {
                LOG(LL_ERROR,
        ...
        case MG_EV_HTTP_PART_END: {
            ...
            if (mp->status >= 0 && fus->fp != NULL) {
                ...
                /*
                 * mp->status < 0 means connection was terminated, so no reason
                 * to send HTTP reply
                 */
            }
    ...
    struct mg_str* host)
{
    ...
    if (use_ssl) {
         ...
         * verification.
         */
        if (opts.ssl_ca_cert == NULL) { opts.ssl_ca_cert = "*"; }
#else
    ...
    if ((nc = mg_connect_opt(
             mgr, conn_addr, MG_CB(ev_handler, user_data), opts)) != NULL) {
    ...
    const char* post_data)
{
    struct mg_connect_opts opts;
    memset(&opts, 0, sizeof(opts));
    return mg_connect_http_opt(
        mgr, MG_CB(ev_handler, user_data), opts, url, extra_headers, post_data);
}
    ...
    for (n = bl; (ll = mg_get_line_len(buf + n, hl - n)) > 0; n += ll) {
        if (mg_ncasecmp(cd, buf + n, cdl) == 0) {
            ...
            header.len = ll - (cdl + 2);
            {
                ...
                mg_http_parse_header2(
                    &header, "name", &var_name2, var_name_len);
            ...
            }
            {
                ...
                mg_http_parse_header2(
                    &header, "filename", &file_name2, file_name_len);
                /* TODO: handle reallocated buffer correctly */
...
mg_abs_path(const char* utf8_path, char* abs_path, size_t len)
{
    ...
    WideCharToMultiByte(
        CP_UTF8, 0, buf2, wcslen(buf2) + 1, abs_path, len, 0, 0);
}
    ...
    struct mg_cgi_env_block* blk)
{
    ...
    mg_addenv(
        ...
        (int)hm->query_string.len,
        hm->query_string.p);

    mg_conn_addr_to_str(
        nc, buf, sizeof(buf), MG_SOCK_STRINGIFY_REMOTE | MG_SOCK_STRINGIFY_IP);
    ...
    if (path_info != NULL && path_info->len > 0) {
        ...
        mg_addenv(
            blk, "PATH_TRANSLATED=%.*s", (int)path_info->len, path_info->p);
    }
    ...
    void* ev_data MG_UD_ARG(void* user_data))
{
    ...
    switch (ev) {
        case MG_EV_RECV:
            /*
             * CGI script does not output reply line, like "HTTP/1.1 CODE
             * XXXXX\n" It outputs headers, then body. Headers might include
             * "Status" header, which changes CODE, and it might include
             * "Location" header which changes CODE to 302.
             *
             * Therefore we do not send the output from the CGI script to the
             * user until all CGI headers are received.
             *
             * Here we parse the output from the CGI script, and if all headers
             * has been received, send appropriate reply line, and forward all
    ...
    const struct mg_serve_http_opts* opts)
{
    ...
    if (mg_start_process(
            opts->cgi_interpreter, prog, blk.buf, blk.vars, dir, fds[1]) != 0) {
        struct mg_connection* cgi_nc =
        ...
        mg_closesocket(fds[0]);
        mg_http_send_error(nc, 500, "CGI failure");
    }

#ifndef _WIN32
    mg_closesocket(
        fds[1]); /* On Windows, CGI stdio thread closes that socket */
#endif
}
...
mg_send_file_data(struct mg_connection* nc, FILE* fp)
{
    char buf[BUFSIZ];
    size_t n;
    while ((n = mg_fread(buf, 1, sizeof(buf), fp)) > 0) { mg_send(nc, buf, n); }
}
    ...
    const struct mg_serve_http_opts* opts)
{
    ...
    if (sscanf(tag, " virtual=\"%[^\"]\"", file_name) == 1) {
        ...
        sscanf(tag, " file=\"%[^\"]\"", file_name) == 1 ||
        sscanf(tag, " \"%[^\"]\"", file_name) == 1) {
        /* File name is relative to the currect document */
        snprintf(path, sizeof(path), "%s", ssi);
        if ((p = strrchr(path, DIRSEP)) != NULL) { p[1] = '\0'; }
        snprintf(
            path + strlen(path), sizeof(path) - strlen(path), "%s", file_name);
    } else {
    ...
    if ((fp = mg_fopen(path, "rb")) == NULL) {
        mg_printf(
            nc,
            "SSI include error: mg_fopen(%s): %s",
            path,
...
do_ssi_exec(struct mg_connection* nc, char* tag)
{
    ...
    if (sscanf(tag, " \"%[^\"]\"", cmd) != 1) {
        ...
        mg_printf(
            nc, "Cannot SSI #exec: [%s]: %s", cmd, strerror(mg_get_errno()));
    } else {
        mg_send_file_data(nc, fp);
        pclose(fp);
    }
    ...
    while ((ch = fgetc(fp)) != EOF) {
        if (in_ssi_tag && ch == '>' && buf[len - 1] == '-' &&
            ...
            if (strncmp(p, d_include.p, d_include.len) == 0) {
                mg_do_ssi_include(
                    nc, hm, path, p + d_include.len + 1, include_level, opts);
            ...
            in_ssi_tag = 1;
            if (len > 0) { mg_send(nc, buf, (size_t)len); }
            len = 0;
            buf[len++] = ch & 0xff;
        } else if (in_ssi_tag) {
...
mg_is_dav_request(const struct mg_str* s)
{
    ...
    for (i = 0; i < ARRAY_SIZE(methods); i++) {
        if (mg_vcmp(s, methods[i]) == 0) { return 1; }
    }
...
mg_print_props(struct mg_connection* nc, const char* name, cs_stat_t* stp)
{
    ...
    mg_printf(
        ...
        "</d:response>\n",
        name_esc.p,
        S_ISDIR(stp->st_mode) ? "<d:collection/>" : "",
        (int64_t)stp->st_size,
        mtime);
...
mg_remove_directory(const struct mg_serve_http_opts* opts, const char* dir)
{
    ...
    while ((dp = readdir(dirp)) != NULL) {
        if (mg_is_file_hidden((const char*)dp->d_name, opts, 1)) { continue; }
...
static int
mg_is_ws_control_frame(unsigned char flags)
{
    ...
    struct websocket_message* wsm)
{
    if (wsm->flags & 0x8) {
        mg_call(
            nc, nc->handler, nc->user_data, MG_EV_WEBSOCKET_CONTROL_FRAME, wsm);
    } else {
        mg_call(nc, nc->handler, nc->user_data, MG_EV_WEBSOCKET_FRAME, wsm);
    }
}

static struct mg_ws_proto_data*
mg_ws_get_proto_data(struct mg_connection* nc)
{
 ...
 */
static void
mg_ws_close(struct mg_connection* nc, const void* data, size_t len)
{
    if ((int)len == ~0) { len = strlen((const char*)data); }
...
mg_deliver_websocket_data(struct mg_connection* nc)
{
    ...
    if (wsd->reass_len > 0) {
         ...
         * fragment.
         *
         * NOTE: it doesn't mean that the current message must be a
         * continuation: it might be a control frame (Close, Ping or Pong),
         * which should be handled without breaking the fragmented message.
         */
    ...
    if (reass && mg_is_ws_control_frame(flags)) {
        ...
        new_data_len > 0 && !reass && !mg_is_ws_control_frame(flags) &&
        wsd->reass_len > 0) {
        /*
         ...
         */
        mg_ws_close(
            nc, "non-continuation in the middle of a fragmented message", ~0);
        return 0;
    }
    ...
    if (ok) {
        ...
        if (mask_len > 0) {
            for (i = 0; i < data_len; i++) {
                new_data[i + header_len] ^=
                    (new_data + header_len - mask_len)[i % 4];
            }
        }
        ...
        if (reass) {
            ...
            if (mg_is_ws_first_fragment(flags)) {
                /*
                 * On the first fragmented frame, skip the first byte (op) and
                 * also reset size to 1 (op), it'll be incremented with the data
                 * len below.
                 */
                new_data += 1;
                wsd->reass_len = 1 /* op */;
            ...
            /*
             * This is a complete message, not a fragment. It might happen in
             * between of a fragmented message (in this case, WebSocket protocol
             * requires current message to be a control frame).
             */
            ...
            switch (flags & FLAGS_MASK_OP) {
                case WEBSOCKET_OP_PING:
                    mg_send_websocket_frame(
                        nc, WEBSOCKET_OP_PONG, wsm.data, wsm.size);
                    break;
            ...
            /* Cleanup frame */
            memmove(
                nc->recv_mbuf.buf + wsd->reass_len,
    ...
    size_t len)
{
    ...
    if (op == WEBSOCKET_OP_CLOSE) { nc->flags |= MG_F_SEND_AND_CLOSE; }
}
    ...
    int strvcnt)
{
    ...
    int i;
    int len = 0;
    for (i = 0; i < strvcnt; i++) { len += strv[i].len; }

    mg_send_ws_header(nc, op, len, &ctx);

    for (i = 0; i < strvcnt; i++) { mg_send(nc, strv[i].p, strv[i].len); }
        ...
        case MG_EV_POLL:
            /* Ping idle websocket connections */
            {
                ...
                if (nc->flags & MG_F_IS_WEBSOCKET &&
                    now >
                        nc->last_io_time + MG_WEBSOCKET_PING_INTERVAL_SECONDS) {
    ...
    const char* extra_headers)
{
    mg_send_websocket_handshake3(
        nc, path, host, protocol, extra_headers, NULL, NULL);
}
    ...
    const struct mg_str pass)
{
    ...
    mbuf_init(&auth, 0);
    if (user.len > 0) { mg_basic_auth_header(user, pass, &auth); }
    ...
    const char* extra_headers)
{
    struct mg_connect_opts opts;
    memset(&opts, 0, sizeof(opts));
    return mg_connect_ws_opt(
        mgr, MG_CB(ev_handler, user_data), opts, url, protocol, extra_headers);
}
...
size_t
mg_fread(void* ptr, size_t size, size_t count, FILE* f)
{
    return fread(ptr, size, count, f);
}

size_t
mg_fwrite(const void* ptr, size_t size, size_t count, FILE* f)
{
...
int
mg_conn_addr_to_str(struct mg_connection* nc, char* buf, size_t len, int flags)
{
    union socket_address sa;
    memset(&sa, 0, sizeof(sa));
    ...
    int ev)
{
    ...
    const char* tag = NULL;
    switch (ev) {
        case MG_EV_RECV: tag = "<-"; break;
        case MG_EV_SEND: tag = "->"; break;
        case MG_EV_ACCEPT: tag = "<A"; break;
        case MG_EV_CONNECT: tag = "C>"; break;
        case MG_EV_CLOSE: tag = "XX"; break;
    }
    ...
    mg_conn_addr_to_str(
        nc, src, sizeof(src), MG_SOCK_STRINGIFY_IP | MG_SOCK_STRINGIFY_PORT);
    mg_conn_addr_to_str(
        nc,
        dst,
        sizeof(dst),
...
struct mg_str
mg_url_encode(const struct mg_str src)
{
        ...
        case MG_MQTT_CMD_CONNECT: {
            ...
            LOG(LL_DEBUG,
                ("%d %2x %d proto [%.*s] client_id [%.*s] will_topic [%.*s] "
                 ...
                 (int)len,
                 (int)mm->connect_flags,
                 (int)mm->keep_alive_timer,
                 (int)mm->protocol_name.len,
                 mm->protocol_name.p,
                 (int)mm->client_id.len,
                 mm->client_id.p,
                 (int)mm->will_topic.len,
                 mm->will_topic.p,
                 (int)mm->will_message.len,
                 mm->will_message.p,
                 (int)mm->user_name.len,
                 mm->user_name.p,
                 (int)mm->password.len,
                 mm->password.p));
            break;
            ...
            while (1) {
                ...
                if (len < 0) {
                    if (len == MG_MQTT_ERROR_MALFORMED_MSG) {
                        ...
                        /* Not fully buffered, let's check if we have a chance
                ...
                nc->handler(
                    nc, MG_MQTT_EVENT_BASE + mm.cmd, &mm MG_UD_ARG(user_data));
                mbuf_remove(io, len);
            }
            break;
        }
...
static struct mg_str
mg_mqtt_next_topic_component(struct mg_str* topic)
{
...
int
mg_mqtt_match_topic_expression(struct mg_str exp, struct mg_str topic)
{
    struct mg_str ec, tc;
    ...
    while (1) {
        ...
        if (mg_strcmp(ec, tc) != 0) { return 0; }
    }
    return (tc.len == 0 && topic.len == 0);
}

int
mg_mqtt_vmatch_topic_expression(const char* exp, struct mg_str topic)
{
    ...
    struct mg_send_mqtt_handshake_opts opts)
{
    ...
    if (client_id != NULL) { id_len = strlen(client_id); }

    total_len = 7 + 1 + 2 + 2 + id_len;

    if (opts.user_name != NULL) { opts.flags |= MG_MQTT_HAS_USER_NAME; }
    if (opts.password != NULL) { opts.flags |= MG_MQTT_HAS_PASSWORD; }
    ...
    if (opts.keep_alive == 0) { opts.keep_alive = 60; }

    if (opts.flags & MG_MQTT_HAS_WILL) { total_len += 2 + wt_len + 2 + wm_len; }
    ...
    if (pd != NULL) { pd->keep_alive = opts.keep_alive; }
}
    ...
    size_t len)
{
    ...
    if (MG_MQTT_GET_QOS(flags) > 0) { total_len += 2; }
    ...
    uint16_t message_id)
{
    ...
    for (i = 0; i < topics_len; i++) { total_len += 2 + strlen(topics[i]); }
    ...
    uint16_t message_id)
{
    ...
    for (i = 0; i < qoss_len; i++) { mg_send(nc, &qoss[i], 1); }
}
    ...
    struct mg_mqtt_message* msg)
{
    ...
    for (s = mg_mqtt_next(brk, NULL); s != NULL; s = mg_mqtt_next(brk, s)) {
        for (i = 0; i < s->num_subscriptions; i++) {
            if (mg_mqtt_vmatch_topic_expression(
                    s->subscriptions[i].topic, msg->topic)) {
                char buf[100], *p = buf;
                mg_asprintf(
                    &p, sizeof(buf), "%.*s", (int)msg->topic.len, msg->topic.p);
                if (p == NULL) { return; }
                mg_mqtt_publish(
                    s->nc, p, 0, 0, msg->payload.p, msg->payload.len);
                if (p != buf) { MG_FREE(p); }
                break;
            }
        }
    ...
    size_t data_len)
{
    switch (rr->rtype) {
        case MG_DNS_A_RECORD:
            if (data_len < sizeof(struct in_addr)) { return -1; }
...
mg_dns_encode_name(struct mbuf* io, const char* name, size_t len)
{
    ...
    do {
        if ((s = strchr(name, '.')) == NULL) { s = name + len; }

        if (s - name > 127) { return -1; /* TODO(mkm) cover */ }
    ...
    size_t rlen)
{
    ...
    if (rr->kind == MG_DNS_INVALID_RECORD) { return -1; /* LCOV_EXCL_LINE */ }

    if (mg_dns_encode_name(io, name, nlen) == -1) { return -1; }
    ...
    int dst_len)
{
    ...
    while ((chunk_len = *data++)) {
        ...
        if ((chunk_len & 0xc0) == 0xc0) {
            ...
            if (off >= msg->pkt.len) { return 0; }
            /* Basic circular loop avoidance: allow up to 16 pointer hops. */
            if (++num_ptrs > 15) { return 0; }
            data = (unsigned char*)msg->pkt.p + off;
            continue;
        }
        if (chunk_len > 63) { return 0; }
        if (chunk_len > leeway) { chunk_len = leeway; }

        if (data + chunk_len >= end) { return 0; }

        memcpy(dst, data, chunk_len);
        data += chunk_len;
        dst += chunk_len;
        leeway -= chunk_len;
        if (leeway == 0) { return dst - old_dst; }
        *dst++ = '.';
    }

    if (dst != old_dst) { *--dst = 0; }
    return dst - old_dst;
}
    ...
    void* ev_data MG_UD_ARG(void* user_data))
{
    ...
    switch (ev) {
        case MG_EV_RECV:
            if (!(nc->flags & MG_F_UDP)) { mbuf_remove(&nc->recv_mbuf, 2); }
    ...
    size_t rdata_len)
{
    ...
    *ans = *question;
    ...
    if (mg_dns_encode_record(
            reply->io, ans, name, strlen(name), rdata, rdata_len) == -1) {
        return -1; /* LCOV_EXCL_LINE */
    };
...
mg_get_ip_address_of_nameserver(char* name, size_t name_len)
{
    ...
    if ((err = RegOpenKeyExW(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &hKey)) !=
        ...
        for (ret = -1, i = 0; 1; i++) {
            ...
            if (RegEnumKeyExW(
                    hKey, i, subkey, &subkey_size, NULL, NULL, NULL, NULL) !=
                ERROR_SUCCESS) {
                break;
            }
            if (RegOpenKeyExW(hKey, subkey, 0, KEY_READ, &hSub) ==
                    ERROR_SUCCESS &&
                ((RegQueryValueExW(
                      hSub, L"NameServer", 0, &type, (void*)value, &len) ==
                      ERROR_SUCCESS &&
                  value[0] != '\0') ||
                 (RegQueryValueExW(
                      hSub, L"DhcpNameServer", 0, &type, (void*)value, &len) ==
                      ERROR_SUCCESS &&
                  value[0] != '\0'))) {
                /*
...
mg_resolve_from_hosts_file(const char* name, union socket_address* usa)
{
    ...
    if ((fp = mg_fopen(MG_HOSTS_FILE_NAME, "r")) == NULL) { return -1; }
    ...
    void* data MG_UD_ARG(void* user_data))
{
    ...
    switch (ev) {
        ...
        case MG_EV_TIMER:
            ...
            break;
        case MG_EV_CLOSE:
            /* If we got here with request still not done, fire an error
             * callback. */
            if (req != NULL) {
                char addr[32];
                mg_sock_addr_to_str(
                ...
                LOG(LL_ERROR,
                    ("Failed to resolve '%s', server %s", req->name, addr));
#endif
    ...
    struct mg_resolve_async_opts opts)
{
    ...
    if (nameserver == NULL) { nameserver = mgr->nameserver; }
    ...
    if (nameserver == NULL) {
        if (mg_get_ip_address_of_nameserver(
                dns_server_buff, sizeof(dns_server_buff)) != -1) {
    ...
    if (opts.dns_conn != NULL) { *opts.dns_conn = dns_nc; }

    return 0;
}

void
mg_set_nameserver(struct mg_mgr* mgr, const char* nameserver)
{
    MG_FREE((char*)mgr->nameserver);
    mgr->nameserver = NULL;
    if (nameserver != NULL) { mgr->nameserver = strdup(nameserver); }
}
    ...
    if (cm->options == NULL) {
        ...
        if (cm->optiomg_tail->number <= new_option->number) {
            ...
            while (current_opt != NULL) {
                if (current_opt->number > new_option->number) { break; }
...
mg_coap_parse(struct mbuf* io, struct mg_coap_message* cm)
{
    ...
    if ((ptr = coap_get_token(ptr, io, cm)) == NULL) { return cm->flags; }

    if ((ptr = coap_get_options(ptr, io, cm)) == NULL) { return cm->flags; }
...
coap_calculate_packet_size(struct mg_coap_message* cm, size_t* len)
{
    ...
    *len = 4; /* header */
    ...
    if (cm->token.len > 8) { return MG_COAP_ERROR | MG_COAP_TOKEN_FIELD; }
    if (cm->code_class > 7) { return MG_COAP_ERROR | MG_COAP_CODE_CLASS_FIELD; }
...
mg_coap_compose(struct mg_coap_message* cm, struct mbuf* io)
{
    ...
    *ptr = (cm->code_class << 5) | (cm->code_detail);
    ...
    while (opt != NULL) {
        ...
        size_t opt_delta_len = coap_split_opt(
            opt->number - prev_opt_number, &delta_base, &delta_ext);
...
mg_coap_send_message(struct mg_connection* nc, struct mg_coap_message* cm)
{
    ...
    if (compose_res != 0) { return compose_res; /* LCOV_EXCL_LINE */ }
    ...
    switch (ev) {
        case MG_EV_RECV:
            ...
            if ((parse_res & MG_COAP_IGNORE) == 0) {
                ...
                nc->handler(
                    nc,
                    MG_COAP_EVENT_BASE + cm.msg_type,
...
{
    /* supports UDP only */
    if ((nc->flags & MG_F_UDP) == 0) { return -1; }
    ...
    switch (ev) {
        case MG_EV_RECV: {
            if (mg_sntp_parse_reply(io->buf, io->len, &msg) < 0) {
                ...
                c->handler(
                    c, MG_SNTP_MALFORMED_REPLY, NULL MG_UD_ARG(user_data));
            } else {
...
{
    if ((c->flags & MG_F_UDP) == 0) { return -1; }
    ...
    const char* sntp_server_name)
{
    ...
    if (tmp != NULL && *(tmp + 1) == '/') { tmp = strchr(tmp + 1, ':'); }

    if (tmp == NULL) { port = ":123"; }

    /* Add udp:// if needed */
    if (strncmp(sntp_server_name, "udp://", 6) != 0) { proto = "udp://"; }
    ...
    const char* sntp_server_name)
{
    ...
    c = mg_sntp_connect(
        mgr, MG_CB(mg_sntp_util_ev_handler, sd), sntp_server_name);
    if (c == NULL) {
        MG_FREE(sd);
        return NULL;
    }
 ...
 */
static void
mg_socks5_handshake(struct mg_connection* c)
{
    ...
    if (r->buf[0] != MG_SOCKS_VERSION) {
        ...
        unsigned char reply[2] = { MG_SOCKS_VERSION,
                                   MG_SOCKS_HANDSHAKE_FAILURE };
        int i;
...
static void
disband(struct mg_connection* c)
{
...
static void
relay_data(struct mg_connection* c)
{
    struct mg_connection* c2 = (struct mg_connection*)c->user_data;
    if (c2 != NULL) {
...
static void
serv_ev_handler(struct mg_connection* c, int ev, void* ev_data)
{
    if (ev == MG_EV_CLOSE) {
        disband(c);
    } else if (ev == MG_EV_RECV) {
...
static void
mg_socks5_connect(struct mg_connection* c, const char* addr)
{
 ...
 */
static void
mg_socks5_handle_request(struct mg_connection* c)
{
    struct mbuf* r = &c->recv_mbuf;
    ...
    if (r->len < 8)
        return; /* return if not fully buffered. min DST.ADDR is 2 */
    ver = p[0];
    cmd = p[1];
    atyp = p[3];
...
static void
socks_handler(struct mg_connection* c, int ev, void* ev_data)
{
    if (ev == MG_EV_RECV) {
...
void
mg_set_protocol_socks(struct mg_connection* c)
{
...
int
slfs_open(const unsigned char* fname, uint32_t flags, uint32_t* token)
{
    _i32 fh;
...
static struct sl_file_open_info* fs_slfs_find_foi(
    const char* name,
    bool create);
...
sl_fs_to_errno(_i32 r)
{
    ...
    switch (r) {
        case SL_FS_OK: return 0;
        case SL_ERROR_FS_FILE_NAME_EXIST: return EEXIST;
        case SL_ERROR_FS_WRONG_FILE_NAME: return EINVAL;
        case SL_ERROR_FS_NO_AVAILABLE_NV_INDEX:
        case SL_ERROR_FS_NOT_ENOUGH_STORAGE_SPACE: return ENOSPC;
        case SL_ERROR_FS_FAILED_TO_ALLOCATE_MEM: return ENOMEM;
        case SL_ERROR_FS_FILE_NOT_EXISTS: return ENOENT;
        case SL_ERROR_FS_NOT_SUPPORTED: return ENOTSUP;
    }
    return ENXIO;
}
...
fs_slfs_lseek(int fd, off_t offset, int whence)
{
    ...
    switch (whence) {
        case SEEK_SET: s_sl_fds[fd].pos = offset; break;
        case SEEK_CUR: s_sl_fds[fd].pos += offset; break;
        case SEEK_END: return set_errno(ENOTSUP);
    }
    return 0;
}
...
static struct sl_file_open_info*
fs_slfs_find_foi(const char* name, bool create)
{
    int i = 0;
    ...
    if (i == MAX_OPEN_SLFS_FILES) { i = 0; /* Evict a random slot. */ }
    if (s_sl_file_open_infos[i].name != NULL) {
...
void
fs_slfs_set_file_size(const char* name, size_t size)
{
...
void
fs_slfs_set_file_flags(const char* name, uint32_t flags, uint32_t* token)
{
    struct sl_file_open_info* foi = fs_slfs_find_foi(name, true /* create */);
    foi->flags = flags;
    foi->token = token;
...
void
fs_slfs_unset_file_flags(const char* name)
{
...
drop_dir(const char* fname, bool* is_slfs)
{
     ...
     */
    if (fname[0] == '/' && strchr(fname + 1, '/') == NULL) { fname++; }
    return fname;
}
...
_close(int fd)
{
    ...
    int r = -1;
    switch (fd_type(fd)) {
        case FD_INVALID: r = set_errno(EBADF); break;
        case FD_SYS: r = set_errno(EACCES); break;
#ifdef CC3200_FS_SPIFFS
        case FD_SPIFFS: r = fs_spiffs_close(fd - SPIFFS_FD_BASE); break;
#endif
#ifdef MG_FS_SLFS
        case FD_SLFS: r = fs_slfs_close(fd - SLFS_FD_BASE); break;
#endif
    }
    DBG(("close(%d) = %d", fd, r));
    return r;
...
_lseek(int fd, off_t offset, int whence)
{
    ...
    switch (fd_type(fd)) {
        case FD_INVALID: r = set_errno(EBADF); break;
        case FD_SYS: r = set_errno(ESPIPE); break;
#ifdef CC3200_FS_SPIFFS
        case FD_SPIFFS:
...
#ifdef CC3200_FS_SPIFFS
        case FD_SPIFFS: r = fs_spiffs_fstat(fd - SPIFFS_FD_BASE, s); break;
#endif
#ifdef MG_FS_SLFS
        case FD_SLFS: r = fs_slfs_fstat(fd - SLFS_FD_BASE, s); break;
#endif
    }
    DBG(("fstat(%d) = %d", fd, r));
    return r;
...
#endif
#ifdef MG_FS_SLFS
        case FD_SLFS: r = fs_slfs_read(fd - SLFS_FD_BASE, buf, count); break;
#endif
    }
...
#endif
#ifdef MG_FS_SLFS
        case FD_SLFS: r = fs_slfs_write(fd - SLFS_FD_BASE, buf, count); break;
#endif
    }
    return r;
}
...
inet_ntop(int af, const void* src, char* dst, socklen_t size)
{
    ...
    res = snprintf(
        ...
        size,
        "%lu.%lu.%lu.%lu",
        SL_IPV4_BYTE(in->s_addr, 0),
        SL_IPV4_BYTE(in->s_addr, 1),
        SL_IPV4_BYTE(in->s_addr, 2),
...
static sock_t mg_open_listening_socket(
    struct mg_connection* nc,
    union socket_address* sa,
    int type,
    int proto);
...
static void
mg_sl_if_connect_tcp(struct mg_connection* nc, const union socket_address* sa)
{
    int proto = 0;
...
mg_sl_if_listen_udp(struct mg_connection* nc, union socket_address* sa)
{
    ...
    if (sock == MG_INVALID_SOCKET) return (errno ? errno : 1);
    mg_sock_set(nc, sock);
    return 0;
}
...
static int
mg_sl_if_tcp_recv(struct mg_connection* nc, void* buf, size_t len)
{
...
mg_sl_if_destroy_conn(struct mg_connection* nc)
{
    ...
    if (!(nc->flags & MG_F_UDP) || nc->listener == NULL) { sl_Close(nc->sock); }
    nc->sock = MG_INVALID_SOCKET;
}
...
#if SL_MAJOR_VERSION_NUM >= 2
                    /* Per SWRU455, this error does not mean verification
                     * failed, it only means that the cert used is not present
                     * in the trusted
...
mg_sl_if_poll(struct mg_iface* iface, int timeout_ms)
{
    ...
    for (nc = mgr->active_connections, num_fds = 0; nc != NULL; nc = tmp) {
        ...
        if (nc->sock != MG_INVALID_SOCKET) {
            ...
            if (!(nc->flags & MG_F_WANT_WRITE) &&
                ...
                if (max_fd == MG_INVALID_SOCKET || nc->sock > max_fd)
                    max_fd = nc->sock;
            }
    ...
    if (num_timers > 0) {
        double timer_timeout_ms =
            (min_timer - mg_time()) * 1000 + 1 /* rounding */;
        if (timer_timeout_ms < timeout_ms) { timeout_ms = timer_timeout_ms; }
    }
    if (timeout_ms < 0) timeout_ms = 0;
            ...
            if (num_ev > 0) {
                fd_flags =
                    (SL_SOCKET_FD_ISSET(nc->sock, &read_set) &&
                         ...
                         ? _MG_F_FD_CAN_READ
                         : 0) |
                    (SL_SOCKET_FD_ISSET(nc->sock, &write_set)
                         ? _MG_F_FD_CAN_WRITE
                         : 0) |
                    (SL_SOCKET_FD_ISSET(nc->sock, &err_set) ? _MG_F_FD_ERROR
                                                            : 0);
            }
...
sl_restart_cb(struct mg_mgr* mgr)
{
    ...
    struct mg_connection* nc;
    for (nc = mg_next(mgr, NULL); nc != NULL; nc = mg_next(mgr, nc)) {
        if (nc->sock == MG_INVALID_SOCKET) continue; /* Could be a timer */
        if (nc->flags & MG_F_LISTENING) {
            DBG(
                ("restarting %p %s:%d",
                 nc,
...
pem_to_der(const char* pem_file, const char* der_file)
{
    ...
    fs_slfs_set_file_size(
        der_file + MG_SSL_IF_SIMPLELINK_SLFS_PREFIX_LEN, 2048);
    df = fopen(der_file, "w");
...
sl_pem2der(const char* pem_file)
{
    ...
    if (pem_ext == NULL || *(pem_ext + 4) != '\0') { return strdup(pem_file); }
    char* der_file = NULL;
...
int
sl_set_ssl_opts(int sock, struct mg_connection* nc)
{
    int err;
...
void
mg_lwip_netif_run_on_tcpip(void (*fn)(void*), void* arg)
{
...
mg_lwip_set_keepalive_params(
    ...
    int count)
{
    if (nc->sock == MG_INVALID_SOCKET || nc->flags & MG_F_UDP) { return; }
...
static err_t
mg_lwip_tcp_recv_cb(void* arg, struct tcp_pcb* tpcb, struct pbuf* p, err_t err)
{
    struct mg_connection* nc = (struct mg_connection*)arg;
...
static void
mg_lwip_if_connect_tcp_tcpip(void* arg)
{
...
void
mg_lwip_if_connect_tcp(struct mg_connection* nc, const union socket_address* sa)
{
    ...
    size_t* sa_len)
{
     ...
     * Address pbuf followed by exactly one data pbuf (recv_cb took care of
     * that).
     */
    int res = 0;
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
    if (nc->sock == MG_INVALID_SOCKET) return -1;
    mgos_lock();
    if (cs->rx_chain != NULL) {
...
static void
mg_lwip_if_connect_udp_tcpip(void* arg)
{
    struct mg_connection* nc = (struct mg_connection*)arg;
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
...
static void
tcp_close_tcpip(void* arg)
{
    tcp_close((struct tcp_pcb*)arg);
}

void
mg_lwip_handle_accept(struct mg_connection* nc)
{
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
...
static void
mg_lwip_if_listen_tcp_tcpip(void* arg)
{
...
static void
mg_lwip_if_listen_udp_tcpip(void* arg)
{
    struct mg_lwip_if_listen_ctx* ctx = (struct mg_lwip_if_listen_ctx*)arg;
    struct mg_connection* nc = ctx->nc;
    union socket_address* sa = ctx->sa;
...
static void
tcp_output_tcpip(void* arg)
{
    tcp_output((struct tcp_pcb*)arg);
}

static void
mg_lwip_tcp_write_tcpip(void* arg)
{
...
static int
mg_lwip_if_udp_send(struct mg_connection* nc, const void* data, size_t len)
{
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
    if (nc->sock == MG_INVALID_SOCKET || cs->pcb.udp == NULL) return -1;
    ...
    struct udp_sendto_ctx ctx = {
        .upcb = upcb, .p = p, .ip = &ip, .port = port
    };
...
static int
mg_lwip_if_can_send(struct mg_connection* nc, struct mg_lwip_conn_state* cs)
{
    int can_send = 0;
    if (nc->send_mbuf.len > 0 || (nc->flags & MG_F_WANT_WRITE)) {
...
static int
mg_lwip_if_tcp_recv(struct mg_connection* nc, void* buf, size_t len)
{
    int res = 0;
    char* bufp = buf;
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
...
{
    memset(sa, 0, sizeof(*sa));
    if (nc == NULL || nc->sock == MG_INVALID_SOCKET) return;
    struct mg_lwip_conn_state* cs = (struct mg_lwip_conn_state*)nc->sock;
    if (nc->flags & MG_F_UDP) {
        struct udp_pcb* upcb = cs->pcb.udp;
        if (remote) {
    ...
    for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
        ...
        if (nc->sock != MG_INVALID_SOCKET) {
            ...
            if (cs->rx_chain != NULL) {
                ...
                /*
                 * If the connection is about to close, and rx_chain is finally
                 * empty, send the MG_SIG_CLOSE_CONN signal
                 */
                mg_lwip_post_signal(MG_SIG_CLOSE_CONN, nc);
            }
        }
...
_wstati64(const wchar_t* path, cs_stat_t* st)
{
    ...
    if (fa == INVALID_FILE_ATTRIBUTES) { return -1; }
    memset(st, 0, sizeof(*st));
...
mg_accept_conn(struct mg_connection* lc)
{
    ...
    if (!TCPIP_TCP_SocketInfoGet((TCP_SOCKET)nc->sock, &si)) { return 0; }
...
mg_pic32_if_poll(struct mg_iface* iface, int timeout_ms)
{
    ...
    for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
        ...
        if (nc->flags & MG_F_CONNECTING) {
            ...
            if (nc->send_mbuf.len != 0) { mg_handle_send(nc); }
    ...
    const union socket_address* sa)
{
    ...
    nc->err = (nc->sock == MG_INVALID_SOCKET) ? -1 : 0;
}
