...
//         r->frames[FRAME_REQ_DATA_IDX] = d && dlen ? zframe_new(d, dlen) :
//         NULL; if (!(r->frames[FRAME_VER_IDX] && r->frames[FRAME_TYP_IDX] &&
...
// {
//     // Note that if a device has a router, than that means the device
//     connected
...
//     if (d->router.sz) request_router_id_set(*r_p, d->router.id,
//     d->router.sz); if (request_send(*r_p, d->sock) < 0) {
