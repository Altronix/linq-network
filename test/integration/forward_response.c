...
main(int argc, char* argv[])
{
    ...
    linq_io_s* client = linq_io_create(&callbacks, NULL);
    if (!client) {
        fixture_destroy(&fixture);
    ...
    s = linq_io_listen(server, "ipc:///tmp/request");
    if (s == LINQ_ERROR_SOCKET) {
        printf("%s", "[S] Listen Failure!\n");
        fixture_destroy(&fixture);
        linq_io_destroy(&server);
    ...
    s = linq_io_connect(client, "ipc:///tmp/request");
    if (s == LINQ_ERROR_SOCKET) {
    ...
    while (!(pass)) {
        ...
        if (!request_sent && linq_io_device_count(client)) {
