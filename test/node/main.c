...
test_node_create(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
    assert_non_null(d);

    node_send_get(d, "ATX", NULL, NULL);
    node_send_get(d, "ATX", NULL, NULL);
    node_send_get(d, "ATX", NULL, NULL);
    node_send_get(d, "ATX", NULL, NULL);
    node_send_post(d, "ATX", "{\"test\":1}", NULL, NULL);
    assert_int_equal(node_request_pending_count(d), 5);
...
static void
test_node_send_get_no_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
    ...
    node_send_get(d, "ATX/hardware", NULL, NULL);
...
static void
test_node_send_get_with_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
...
static void
test_node_send_delete_no_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
    ...
    node_send_delete(d, "ATX/hardware", NULL, NULL);
    msg = czmq_spy_mesg_pop_outgoing();
    rid = zmsg_pop(msg);
    ver = zmsg_pop(msg);
    typ = zmsg_pop(msg);
...
static void
test_node_send_delete_with_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
...
static void
test_node_send_post_no_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
    ...
    node_send_post(d, "ATX/hardware", "{\"test\":1}", NULL, NULL);
    msg = czmq_spy_mesg_pop_outgoing();
    rid = zmsg_pop(msg);
    ver = zmsg_pop(msg);
    typ = zmsg_pop(msg);
...
static void
test_node_send_post_with_prefix(void** context_p)
{
    ((void)context_p);
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
...
static void
on_response(void* context, E_LINQ_ERROR e, const char* json, node_s** d)
{
    bool* pass = context;
    *pass = true;
    assert_string_equal(json, "{\"test\":1}");
    assert_string_equal("sid", node_serial(*d));
    assert_int_equal(e, 0);
}

static void
test_node_response(void** context_p)
{
    ((void)context_p);
    bool* pass = false;
    zsock_t* sock = NULL;
    node_s* d = node_create(&sock, (uint8_t*)"rid", 3, "sid", "pid");
    node_send_post(d, "/ATX/hardware", "{\"test\":1}", on_response, &pass);

    // TODO - add similiar test from linq.c which will test the parsing
    assert_int_equal(node_request_pending_count(d), 1);
    node_request_resolve(d, 0, "{\"test\":1}");
...
main(int argc, char* argv[])
{
    ...
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_node_send_get_no_prefix),
        cmocka_unit_test(test_node_send_get_with_prefix),
        cmocka_unit_test(test_node_send_delete_no_prefix),
        cmocka_unit_test(test_node_send_delete_with_prefix),
        cmocka_unit_test(test_node_send_post_no_prefix),
        cmocka_unit_test(test_node_send_post_with_prefix),
        cmocka_unit_test(test_node_response),
    };
